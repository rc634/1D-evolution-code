...
# Create a figure and axis for plotting
fig, ax = plt.subplots()
lines = [ax.plot([], [], lw=2, label='phi.dat')[0], ax.plot([], [], lw=2, label='psi.dat')[0], ax.plot([], [], lw=2, label='V.dat')[0],]  # Create two lines for phi and psi data
...
def update(frame):
    ...
    lines[2].set_data(x_data[frame], v_data[frame])  # Update psi data
    return lines
