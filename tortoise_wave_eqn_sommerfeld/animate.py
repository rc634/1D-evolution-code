...
# Create a figure and axis for plotting
fig, ax = plt.subplots()
lines = [ax.plot([], [], lw=0.5, color='k')[0], ax.plot([], [], lw=2, label='Veff', color='green')[0], ax.plot([], [], lw=2, label=r'$\Psi$', color='red')[0], ax.plot([], [], lw=2, label=r'$\Phi$', color='blue')[0]]  # Create two lines for phi and psi data
...
def update(frame):
    ...
    lines[1].set_data(x_data[frame], v_data[frame])  # Update psi data
    lines[2].set_data(x_data[frame], psi_data[frame])  # Update psi data
    lines[3].set_data(x_data[frame], phi_data[frame])  # Update phi data
