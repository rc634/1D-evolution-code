...
void FieldClass::initialiseDGaussianDX(double mean, double sigma, double a, double v)
{
    for (int i = 0; i < m_gridpoints; i++)
    {
        ...
        m_field[i] *= v*(x - mean)/(sigma*sigma);
    }

}

void FieldClass::initialiseGaussian(double mean, double sigma, double a, double v)
{
    for (int i = 0; i < m_gridpoints; i++)
    {
        double x = getX(i);
...
inline double FieldClass::d2_4th_left(int i)
{
  double deriv=0.;
  deriv += 11.*m_field[i-4] - 56.*m_field[i-3] + 114.*m_field[i-2];
  deriv += -104.*m_field[i-1] + 35.*m_field[i];
  return deriv/(12.*m_dx_sqr);
}

inline double FieldClass::d2_4th_right(int i)
{
    double deriv=0.;
    deriv += 11.*m_field[i+4] - 56.*m_field[i+3] + 114.*m_field[i+2];
    deriv += -104.*m_field[i+1] + 35.*m_field[i];
...
inline double FieldClass::d1_4th_left(int i)
{
  double deriv=0.;
  deriv += 3.*m_field[i-4] - 16.*m_field[i-3] + 36.*m_field[i-2];
  deriv += -48.*m_field[i-1] + 25.*m_field[i];
  return deriv/(12.*m_dx);
}

inline double FieldClass::d1_4th_right(int i)
{
  double deriv=0.;
  deriv -= 3.*m_field[i+4] - 16.*m_field[i+3] + 36.*m_field[i+2];
  deriv -= -48.*m_field[i+1] + 25.*m_field[i];
